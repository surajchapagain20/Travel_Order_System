...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $selected_ph_email = '';
    if (!empty($ph_data)) {
        list($selected_ph_name, $selected_ph_email) = explode('|', $ph_data);
    }
    ...
    if (
        ...
        empty($purpose) || empty($modeOfTransport) || empty($noOfDays) || empty($selected_ph_name)
    ) {
        ...
        if (empty($errorMessage)) {
            $stmt = $conn->prepare("INSERT INTO travel_orders (
                ...
                selected_ph_name, selected_ph_email,
            ...
            if ($stmt) {
                ...
                $stmt->bind_param(
                    ...
                    $approvingAuthority, $document, $selected_ph_name, $selected_ph_email, $currentUser
                );
