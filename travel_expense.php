...
// ── AJAX: Check duplicate travel_order_no for emp ─────────────────────────
if (isset($_GET['check_duplicate_order'])) {
    header('Content-Type: application/json');
    $empID   = trim($_GET['emp_id']          ?? '');
    $orderNo = trim($_GET['travel_order_no'] ?? '');
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT id, status, from_date, to_date, signature_date
            FROM travel_expenses
            WHERE emp_id = ? AND travel_order_no = ?
            LIMIT 1
        ");
        $stmt->execute([$empID, $orderNo]);
    ...
    try {
        ...
        if ($hasFile) {
            ...
            if (!in_array($fileMime, $allowedTypes) || !in_array($fileExt, $allowedExts)) {
                throw new Exception("अमान्य फाइल प्रकार। PDF, JPG, JPEG, वा PNG मात्र अनुमति छ। (Only PDF, JPG, JPEG, PNG allowed.)");
            }

            // ── Validate file size (max 5 MB) ──────────────────────────────
            if ($file['size'] > 5 * 1024 * 1024) {
                throw new Exception("फाइल साइज ५ MB भन्दा बढी हुन सक्दैन। (File size must not exceed 5 MB.)");
            }

            // ── Save file to upload directory ──────────────────────────────
            $uniqueName   = uniqid('doc_', true) . '_' . time() . '.' . $fileExt;
            $destPath     = $uploadDir . $uniqueName;

            if (!move_uploaded_file($file['tmp_name'], $destPath)) {
                throw new Exception("फाइल अपलोड गर्न असफल भयो। (File upload failed — check server permissions.)");
            }

            $documentPath = $destPath;
        } else {
            // Keep the previously uploaded document path
        ...
        // Upsert logic: update if record exists, otherwise insert new
        if (isset($existingId) && $existingId) {
            ...
            $stmt->execute([
                ...
                ':status' => in_array($_POST['expense_status'] ?? '', ['Pending','Approved']) ? $_POST['expense_status'] : 'Pending',
                ':id' => $existingId
            ]);
            $lastId = $existingId;
        } else {
            // -- No explicit edit ID: check for existing record by travel_order_no --
            ...
            if ($dup && strtolower($dup['status']) === 'approved') {
                // Already approved -- block editing
                throw new Exception(
                ...
                // Pending record found -- UPDATE it
                $existingId = $dup['id'];
                $stmt = $pdo->prepare("
                    UPDATE travel_expenses SET
                        emp_id          = :emp_id,
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO travel_expenses
                        (emp_id, name, position, office, purpose, from_date, to_date, vehicle,
                         ...
                         advance, signature_date, remarks, document_path, travel_order_no, status)
                    VALUES
                        (:emp_id, :name, :position, :office, :purpose, :from_date, :to_date, :vehicle,
                         :distance, :fare, :airport, :road_tax, :daily_rate, :days, :hotel, :other_exp,
                         :advance, :signature_date, :remarks, :document_path, :travel_order_no, :status)
        ...
        } // closes outer else block

        $lastId  = $existingId ?? $pdo->lastInsertId();
...
<head>
    ...
    <style>
        ...
        #dupWarnModal.show { display: flex; }
        .dup-warn-box {
            background: #fff;
            border-radius: 12px;
        ...
        .dup-warn-header h3 { margin: 0; font-size: 17px; font-weight: 700; }
        .dup-warn-body {
            padding: 22px 24px 14px;
            text-align: center;
        }
        .dup-warn-body .big-icon { font-size: 50px; display: block; margin-bottom: 10px; }
        .dup-warn-body p { font-size: 15px; color: #333; margin: 0 0 8px; font-weight: 600; }
        .dup-warn-body small { font-size: 13px; color: #666; display: block; margin-bottom: 3px; }
        .dup-warn-footer {
        ...
        .btn-dup-ok:hover { background: #d97706; }
    </style>
</head>
<body>
...
<div class="form-container">
    ...
    <!-- ══ DUPLICATE ORDER WARNING MODAL ══ -->
    <div id="dupWarnModal" role="alertdialog" aria-modal="true">
        <div class="dup-warn-box">
            <div class="dup-warn-header">
                <span style="font-size:26px;line-height:1;">⚠️</span>
                <h3>Duplicate Travel Order!</h3>
            </div>
            <div class="dup-warn-body">
                <span class="big-icon">🔁</span>
                <p id="dup-warn-title">यो Travel Order को खर्च पहिले नै दर्ता भइसकेको छ।</p>
                <small id="dup-warn-detail"></small>
                <small style="color:#92400e;margin-top:6px;display:block;">
                    एउटै Travel Order No मा एउटा मात्र खर्च विवरण दर्ता गर्न सकिन्छ।<br>
                    (Only one expense entry is allowed per Travel Order No.)
                </small>
            </div>
            <div class="dup-warn-footer">
                <button class="btn-dup-ok" id="btn-dup-ok">ठीक छ, फिर्ता जान्छु</button>
            </div>
        </div>
    </div>
    ...
    <div id="documentViewerModal" style="display: none; position: fixed; inset: 0; z-index: 10000; background: rgba(0,0,0,0.7); align-items: center; justify-content: center;">
        <div style="background: #fff; border-radius: 8px; max-width: 900px; width: 90vw; height: 85vh; display: flex; flex-direction: column; overflow: hidden; border: 2px solid #003087;">
            <div style="background: #003087; color: #fff; padding: 12px 20px; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0;">
                <h2 style="margin: 0; font-size: 16px; font-weight: bold; color: #fff;">कागजात / बिल पूर्वावलोकन (Document Preview)</h2>
                <button type="button" id="btn-close-viewer" style="background: none; border: none; color: #fff; font-size: 28px; cursor: pointer; line-height: 1;">&times;</button>
            </div>
            <div id="viewer-body" style="flex: 1; overflow: auto; padding: 10px; display: flex; align-items: center; justify-content: center; background: #eaeaea;">
                <!-- Content will be injected dynamically (iframe or img) -->
            </div>
        </div>
    </div>
    ...
    <div id="mainFormWrap">
        ...
        <form method="POST" enctype="multipart/form-data" id="expenseForm">
            ...
            <input type="hidden" name="expense_status"  id="expense_status" value="Pending">
...
<?php $isApprovedForm = (strtolower($formData['status'] ?? $formData['expense_status'] ?? '') === 'approved'); ?>
<!-- ══ ACTION BUTTONS — outside #mainFormWrap so the overlay never covers them ══ -->
<div style="max-width:1000px;margin:0 auto;text-align:center;padding:20px 0 30px;background:#fff;border:2px solid #000;border-top:none;">
    <button type="button" id="btnSubmit"
            style="padding:12px 30px;font-size:16px;background:#003087;color:#fff;border:none;cursor:pointer;border-radius:4px; <?= $isApprovedForm ? 'display:none;' : '' ?>">
        Save Draft
    </button>
    <button type="button" id="btnPostData"
            style="padding:12px 30px;font-size:16px;background:#28a745;color:#fff;border:none;cursor:pointer;border-radius:4px;margin-left:10px; <?= $isApprovedForm ? 'display:none;' : '' ?>">
        Post Data
    </button>
    <button type="button"
...
fileInput.addEventListener('change', () => {
    if (fileInput.files.length > 0) {
        ...
        docWarnShown = false;   // reset: allow popup again if they later clear the file
    } else {
...
//  DOCUMENT WARNING POPUP
// ══════════════════════════════════════════════════════════════
let docWarnShown = false;   // show the popup only once per page load

function showDocWarn() {
    // Always highlight the file input row
    fileInput.classList.add('file-required');
    document.getElementById('file-label-row').style.background = '#fff5f5';

    if (!docWarnShown) {
        // First time only — show the full popup
        docWarnModal.classList.add('show');
        docWarnShown = true;
    } else {
        // Subsequent attempts — silently scroll to the field, no popup
...
// ══════════════════════════════════════════════════════════════
//  SUBMIT — validate then set status and submit
// ══════════════════════════════════════════════════════════════
function submitFormWithStatus(status) {
    ...
    document.getElementById('expense_status').value = status;
    expenseForm.submit();
}

btnSubmit.addEventListener('click',  () => submitFormWithStatus('Pending'));
btnPostData.addEventListener('click', () => submitFormWithStatus('Approved'));
...
function populateFormFromTravelOrder(d, note) {
    ...
    document.getElementById('btnPostData').style.display = 'inline-block';
    calculateTotals();
}
...
function populateFormFromRecord(rec) {
    ...
    if (rec.status && rec.status.toLowerCase() === 'approved') {
        ...
        document.getElementById('btnPostData').style.display = 'none';
    } else {
        statusEl.className = 'ok';
...
function openDocumentViewer(url) {
    ...
    // Extract file parameter and get extension
    const fileParam = new URLSearchParams(url.split('?')[1]).get('file') || '';
    const ext = fileParam.split('.').pop().toLowerCase();
    
    if (ext === 'pdf') {
        viewerBody.innerHTML = `<iframe src="${url}" style="width: 100%; height: 100%; border: none;"></iframe>`;
    } else if (['jpg', 'jpeg', 'png'].includes(ext)) {
        viewerBody.innerHTML = `<img src="${url}" style="max-width: 100%; max-height: 100%; object-fit: contain; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">`;
    } else {
        viewerBody.innerHTML = `<iframe src="${url}" style="width: 100%; height: 100%; border: none;"></iframe>`;
    }
...
btnCloseViewer.addEventListener('click', closeDocumentViewer);
...
// Event delegation for document view link click
document.addEventListener('click', e => {
    const link = e.target.closest('.view-doc-link');
    if (link) {
        e.preventDefault();
        openDocumentViewer(link.getAttribute('href'));
    }
});
</script>
</body>
